...
**Keepalived** - это инструмент, который позволяет насроить **HA** (High Availability) кластер, тот есть тот, который является отказоустойчивым. Делается это при помощи **VIP** (Virtual IP). Благодаря этому, механизм **failover** работает.
